...
$ # собрать докер
$ ya package \
    --docker crypta/graph/rt/brusilov/docker/package.json \
    --docker-network host \
    --docker-registry registry.yandex.net \
    --docker-repository crypta
$ # проверить список изображений
$ docker image ls registry.yandex.net/crypta/brusilov
$ IMAGE=$(docker image ls registry.yandex.net/crypta/brusilov -q | head -1)
$ # запустить шел
$ docker run \
    --network host \
    -it $IMAGE bash
