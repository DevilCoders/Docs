...
`GreenThread` — thread-like обёртка над гринлетами, `ThreadMixin` — позволяет оффлоадить задачи в отдельный тред.
...
Реализует `ThriftBinClient`. Умеют восстанавливать соединение с сервером
