# Эта сандбокс таска является прямым наследником пакета rotate-s3-backup
И теперь он перенесен в sandbox.
Предполагается, что каждый проект настроет себе
пару шедулеров на ротацию и мониторинг
Исходный пакет вот тут https://github.yandex-team.ru/admins/rotate-s3-backups

# подробно про пакет rotate-s3-backup
https://wiki.yandex-team.ru/media-admin/s3backup/
...
```
ya make
visualizer/visualize_steps --help
usage: visualize-steps [-h] [--start YYYMMDD] --che YYYYMMDD [YYYYMMDD ...]
...
skacheev-nix:rotate-s3-backups (master) λ visualizer/visualize-steps --che 20170906 20171217 20181006 -d 1 -w 5 -m 5 -p 720
