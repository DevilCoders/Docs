...
- Для доступа к заголовкам запроса пользуйтесь функций [get](https://expressjs.com/en/4x/api.html#req.get) запроса, а не объектом headers. Так как это позволяет не беспокоиться насчет регистра имени заголовка.
