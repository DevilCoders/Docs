...
[Проект в error booster](https://error.yandex-team.ru/projects/lavka-pigeon)
...
2. Идём на Juggler в раздел [Notification rules](https://juggler.yandex-team.ru/notification_rules/?query=recipient%3Dlavka-pigeon-alerts), жмём `Создать` и вводим следующие данные

Селекторы (через &):
`namespace=error-booster-alerts`, `tag=error-booster-project-lavka-pigeon`,`tag=error-booster-type-NewErrors`(Count|CountNewErrors|NewErrors|TrendErrors тип алерта из error booster), `service=alert-123`(номер алерта из error booster)

```
namespace=error-booster-alerts
& tag=error-booster-project-lavka-pigeon
& tag=error-booster-type-NewErrors
& service=alert-123
```

Неймспейс: `pigeon frontend` (создать новый можно [тут](https://juggler.yandex-team.ru/namespaces/?query=namespace=pigeon-frontend))

Получатели: `lavka-pigeon-alerts` (чат в телеграме)
