# Legacy Ручки запросов недавних позиций

Мы предоставляем ручки для запроса "недавних позиций" для контракторов. Это ручки
/legacy/position
/legacy/bulk/positions
/legacy/shorttrack
/legacy/bulk/shorttracks
    ...
    post:
        ...
        requestBody:
            ...
            content:
                application/json:
                    schema:
                        $ref: '../definitions.yaml#/components/schemas/PositionRequest'
        responses:
            '200':
                description: Success
                content:
...
SignalSourceOrSet:
    description: |
        A single source registered for a pipeline, or a set of sources. 
        A set is just a registered alias to an array of sources.
    type: string
    pattern: '^[a-zA-Z0-9_=-]+$'
    maxLength: 128
    minLength: 1
...
SignalSources:
    description: Array of sources, and/or named sets
    type: array
    items:
        $ref: "#/components/schemas/SignalSourceOrSet"

Algorithm:
    description: Algorithm to use when getting positions
    type: string
    enum:
      - AsOfNow
      - WithRetry

AsOfNowAlgorithmParams:
    type: object
...
# max_retries ушел, это детали реализации
# весь алгоритм остается только на переходный период
WithRetryAlgorithmParams:
    type: object
    additionalProperties: false
    properties:
        algorithm:
            $ref: "#/components/schemas/Algorithm"
        timeout:
            description: Amount of time to wait between retries
            type: integer
            x-taxi-cpp-type: std::chrono::milliseconds
            minimum: 1
            default: 30
        min_positions_required_count:
            description: Algorithm wouldn't try to get more positions if `min_positions_required_count`
                were found. -1 is for try to find all of them
            type: integer
            minimum: -1
            default: -1
    required:
...
PositionRequest:
    description: Describes requested positions and algorithm
    type: object
    ...
    properties:
        ...
        algorithm:
            oneOf:
              - $ref: "#/components/schemas/AsOfNowAlgorithmParams"
              - $ref: "#/components/schemas/WithRetryAlgorithmParams"
            discriminator:
                propertyName: algorithm
                mapping:
                    AsOfNow: "#/components/schemas/AsOfNowAlgorithmParams"
                    WithRetry: "#/components/schemas/WithRetryAlgorithmParams"
    required:
      - sources
```
...
AndroidGps и если чего-то не хватает, то сделай 3 ретрая
**v1/taxi/position?uuid=u1&dbid=db1**
```yaml
sources:
 - Verified
...
algorithm - это более сложный параметр. Он решает проблему отсутствия данных вообще или отсутствия какого-то источника. Мы пока поддерживаем 2 алгоритма:
 * AsOfNow - возвращает строго данные на момент запроса. Чего нет - того нет.
 * WithRetry - Этот алгоритм попытается собрать как минимум min_positions_required_count, делая внутри алгоритма несколько ретраев. Если даже после всех ретраев нужное кол-во позиций не набралось, то он возвращает то что есть. Ретраи помогают бороться с ситуацией, когда точка еще только бежит по нашей системе и еще не дошла до сервиса, но потребитель уже точно знает что ее отправил.
        ...
        post:
            ...
            requestBody:
                ...
                content:
                    application/json:
                        schema:
                            $ref: '../definitions.yaml#/components/schemas/ShorttrackRequest'
            responses:
                '200':
                    description: Success
                    content:
...
```yaml
ShorttrackRequest:
    description: Describes requested shorttrack
    type: object
    additionalProperties: true
    x-taxi-additional-properties-true-reason: for allOf
    properties:
        alternatives_selector:
            $ref: "#/components/schemas/AlternativesSelector"
        max_age:
            description: Maximum age of the coordinate (in seconds). By default
                - +inf.
            type: integer
