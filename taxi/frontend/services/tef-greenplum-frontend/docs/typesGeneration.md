...
Генерация осуществляется в папку `/static/types/api/<repo_name>/<service_name>` командой `npm run typings`.
...
npm run typings -- --scope backend-py3:crm-admin
```
...
Например:
```typescript
// Файл /static/types/api/repo/service/definitions.ts
...
```
```typescript
// Файл /static/.../api/service/types.ts
