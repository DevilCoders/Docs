...
| `reportPath`                                                           | `E2E_REPORT_PATH`    |              | Путь по которому будет создан html отчет. Этот параметр задается сейчас в TeamCity скриптах запуска тестов, чтобы отчеты для разных тест-сетов попадали в разные папки. |
