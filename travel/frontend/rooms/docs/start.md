...
```
$ git clone https://github.yandex-team.ru/travel-ui/ya-travel.git
$ cd ya-travel
$ cp .env.development.example .env.development
$ npm run dependency:dev
```

Предпочтительно использовать git по ssh. Ключ нужно создать, если его еще нет -
https://wiki.yandex-team.ru/security/ssh/macos/ - и добавить в профиль гитхаба.

Если во время пуша будет приходить 403 вида "Доступ к репозиторию Х запрещен" - попросите доступ в travel-ui.
...
Значение поля `secret` берем из секретницы https://yav.yandex-team.ru - `travel-testing-tvm` (поле clientSecret).
