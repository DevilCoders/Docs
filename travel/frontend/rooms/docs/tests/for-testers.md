...
Перейти в корневую папку проекта `ya-travel`.
_При открытии терминала в WebStorm, папка `ya-travel` открывается по умолчанию._
...
## Настройка учетной записи GitHub Enterprise в WebStorm

Чтобы пушить в репозиторий нужно настроить учетную запись GitHub в WebStorm.

1. Перейдите в раздел с учетными записями GitHub в WebStorm: Settings -> Version Control -> GitHub -> + (Add) -> Log In to GitHub Enterprise.
2. Укажите сервер Яндекса https://github.yandex-team.ru
3. Рядом с полем token нажмите "Generate..." должна открыться предзаполненная страница генерации токена, если не открылась, то авторизуйтесь и попробуйте снова.
4. На странице нажмите кнопку "Generate token" внизу страницы и скопируйте полученный токен в поле token в WebStorm.
5. Нажмите "Add account"
6. На странице с учетными записями GitHub в WebStorm снимите галочку "Clone git repositories using ssh"
