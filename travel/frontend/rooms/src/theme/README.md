...
Т.к. плагин `postcss-theme-fold` заменяет все css переменные на их значения, то отладка на странице становится практически невозможной.

Чтобы вернуть переменные на страницу необходимо выставить в .env.development `DEBUG_LEGO_THEMEKIT=1`. Эта настройка:

1. убирает плагин в client.webpack.config.js;
2. подключает в App.tsx собранный файл с css переменными.
