...
Если вы сгенерировали файл локально, а не на dev машине, то далее доставьте файл на **dev-машину** вашим средством синхронизации (например rsync)
...
Скрипт скопирует созданный `tvmtool.conf` и `local.auth` в `/etc/tvmtool` и перезапустит tvm.
