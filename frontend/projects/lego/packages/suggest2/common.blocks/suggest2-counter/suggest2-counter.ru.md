...
| [type-counts](#mods-type-counts) |	`yes`            |	JS               | Добавление логирования типов в счетчик. |
| [insert-arrow](#mods-insert-arrow)|	`yes`            |	JS               | Добавление логирования стрелок для подстановки в счетчик. |
...
<a name="mods-type-counts"></a>
### Модификатор `type-counts`

Добавляет новый параметр логирования `type_counts`, в котором считается количество показанных подсказок для каждого типа

Тип формируется из параметров подсказки `type` и `subtype`, поддерживается формат подсказки из ручек `/suggest/suggest-ya.cgi?` и `/suggest/suggest-endings?`
1. `["bemjson", text, {bemjson, type, subtype}]` - из значений `type` и `subtype`
2. `[base_type, _, _, _, _, {type}]` - из значений `base_type` и `type`
...
Добавляет новый параметр логирования `use_arrow`, в котором считается количество кликов по стрелке для подстановки
