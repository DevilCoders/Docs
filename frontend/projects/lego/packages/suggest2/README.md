...
Блок                                                          | Описание
------------------------------------------------------------- | -------------------------
[suggest2-form](сommon.blocks/suggest2-form/suggest2-form.ru.md)            | Форма поиска, содержащая все контролы блока `suggest2`. Также используется для первоначальной инициализации блока `suggest2` в DOM-дереве.
[suggest2-item](сommon.blocks/suggest2-item/suggest2-item.ru.md)            | Элемент поисковой подсказки.
[suggest2-input](сommon.blocks/suggest2-input/suggest2-input.ru.md)         | Универсальный API для подключения к `suggest2` блоков `input` из разных библиотек.
[suggest2-popup](сommon.blocks/suggest2-popup/suggest2-popup.ru.md)         | Универсальный API для подключения к `suggest2` блоков `popup` из разных библиотек.
[suggest2-detect](сommon.blocks/suggest2-detect/suggest2-detect.ru.md)      | Блок для сбора данных о браузере пользователя.<br> **NB** _Необходимо [явно декларировать](https://github.yandex-team.ru/search-interfaces/frontend/blob/master/projects/lego/packages/islands/common.blocks/suggest2/suggest2.examples/20-large.bemjson.js#L74) в BEMJSON блока._
[suggest2-provider](сommon.blocks/suggest2-provider/suggest2-provider.ru.md)| Провайдер данных. Поставляет блоку `suggest2` данные о подсказках из набора источников (suggest-сервера).
[suggest2-model](сommon.blocks/suggest2-model/suggest2-model.ru.md)          | Модель данных. Содержит методы работы с данными о подсказках, которые запрашиваются с сервера через [suggest2-provider](сommon.blocks/suggest2-provider/suggest2-provider.ru.md).
[suggest2-view](сommon.blocks/suggest2-view/suggest2-view.ru.md)            | Представление данных. Преобразует данные о подсказках, полученных от [suggest2-model](сommon.blocks/suggest2-model/suggest2-model.ru.md), из формата JSON в BEMJSON. Обеспечивает различные способы представления данных.
[suggest2-counter](сommon.blocks/suggest2-counter/suggest2-counter.ru.md)   | **Необязательный**. Счетчик саджеста. Подсчитывает [различные характеристики](https://beta.wiki.yandex-team.ru/tenorok/suggest/counter/) взаимодействия пользователя с саджестом.
...
| [type](#mods-type)| `simple`, `advanced`, `all` | BEMJSON | Файлы зависимостей для подключения подсказок только определенных [типов](сommon.blocks/suggest2-item/suggest2-item.ru.md#%D0%9C%D0%BE%D0%B4%D0%B8%D1%84%D0%B8%D0%BA%D0%B0%D1%82%D0%BE%D1%80-type) (простого и/или расширенного формата).  |
...
| [feedback](#mods-feedback)   | `yes`           | BEMJSON | Кнопка 'Пожаловаться' в подвале. Доступен на уровне desktop |
...
Определяет файлы c зависимостями, каждый из которых подключает подсказки определенных [типов](common.blocks/suggest2-item/suggest2-item.ru.md). Используется для вывода в результатах запросов саджеста подсказок только нужных типов.
...
<a href="#mods-feedback"></a>
#### Модификатор `feedback`

Добавляет подвал и кнопку `Пожаловаться` в попапе. По клику срабатывает событие `show-feedback` c параметрами `suggestions`, `url`

* `suggestions` - текстовое представление текущих подсказок формата `<suggest[0]>;<suggest[1]>;...;<suggest[10]>`
* `url` - url запроса с параметрами, по которому были получены текущие подсказки
...
Добавляет в подсказки стрелку для подстановки. По клику срабатывает событие `arrowInsert` c параметрами `val`, `domEvent`
...
* Подключается с помощью описания в BEMJSON-декларации (в отличие от первой версии саджеста, блок [suggest](../islands-derpecated/common.blocks/suggest/suggest.ru.md), который [подключается в JavaScript](https://lego.yandex-team.ru/libs/islands/v3.x/desktop/suggest/#Объявление-блока-на-странице-22)).
* Реализация и возможности существенно отличаются от блока [suggest](../islands-derpecated/common.blocks/suggest/suggest.ru.md).
