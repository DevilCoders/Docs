## Блок `tools-header-suggest-item` на desktop
### Использование
```javascript
const dist = require('@yandex-lego/serp-header/dist/base/tools-header-suggest-item.desktop');
...
const scriptSrc = `${staticData.BASE_PATH}base/tools-header-suggest-item/tools-header-suggest-item-base.desktop.client.js`
```
...
`YENV=testing block=tools-header-suggest-item platform=desktop npm run build:dev`
Они соберутся в корне `islands` в `desktop.examples/tools-header-suggest-item`.
Можно увидеть их, запустив в корне `npx http-server` и открыв http://127.0.0.1:8080/desktop.examples/tools-header-suggest-item.
