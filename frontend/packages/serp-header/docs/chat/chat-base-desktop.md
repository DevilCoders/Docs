## Блок `chat` на desktop
### Использование
```javascript
const dist = require('@yandex-lego/serp-header/dist/base/chat.desktop');
...
const scriptSrc = `${staticData.BASE_PATH}base/chat/chat-base.desktop.client.js`
```
...
`YENV=testing block=chat platform=desktop npm run build:dev`
Они соберутся в корне `islands` в `desktop.examples/chat`.
Можно увидеть их, запустив в корне `npx http-server` и открыв http://127.0.0.1:8080/desktop.examples/chat.
