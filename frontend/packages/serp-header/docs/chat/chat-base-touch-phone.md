## Блок `chat` на touch-phone
### Использование
```javascript
const dist = require('@yandex-lego/serp-header/dist/base/chat.touch-phone');
...
const scriptSrc = `${staticData.BASE_PATH}base/chat/chat-base.touch-phone.client.js`
```
...
`YENV=testing block=chat platform=touch-phone npm run build:dev`
Они соберутся в корне `islands` в `touch-phone.examples/chat`.
Можно увидеть их, запустив в корне `npx http-server` и открыв http://127.0.0.1:8080/touch-phone.examples/chat.
