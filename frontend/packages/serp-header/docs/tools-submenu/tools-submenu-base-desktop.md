## Блок `tools-submenu` на desktop
### Использование
```javascript
const dist = require('@yandex-lego/serp-header/dist/base/tools-submenu.desktop');
...
const scriptSrc = `${staticData.BASE_PATH}base/tools-submenu/tools-submenu-base.desktop.client.js`
```
...
`YENV=testing block=tools-submenu platform=desktop npm run build:dev`
Они соберутся в корне `islands` в `desktop.examples/tools-submenu`.
Можно увидеть их, запустив в корне `npx http-server` и открыв http://127.0.0.1:8080/desktop.examples/tools-submenu.
