## Блок `navigation-more` на desktop
### Использование
```javascript
const dist = require('@yandex-lego/serp-header/dist/base/navigation-more.desktop');
...
const scriptSrc = `${staticData.BASE_PATH}base/navigation-more/navigation-more-base.desktop.client.js`
```
...
`YENV=testing block=navigation-more platform=desktop npm run build:dev`
Они соберутся в корне `islands` в `desktop.examples/navigation-more`.
Можно увидеть их, запустив в корне `npx http-server` и открыв http://127.0.0.1:8080/desktop.examples/navigation-more.
