## Блок `tools-header-suggest` на touch-phone
### Использование
```javascript
const dist = require('@yandex-lego/serp-header/dist/base/tools-header-suggest.touch-phone');
...
const scriptSrc = `${staticData.BASE_PATH}base/tools-header-suggest/tools-header-suggest-base.touch-phone.client.js`
```
...
`YENV=testing block=tools-header-suggest platform=touch-phone npm run build:dev`
Они соберутся в корне `islands` в `touch-phone.examples/tools-header-suggest`.
Можно увидеть их, запустив в корне `npx http-server` и открыв http://127.0.0.1:8080/touch-phone.examples/tools-header-suggest.
