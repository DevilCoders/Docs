## Блок `notifier` на desktop
### Использование
```javascript
const dist = require('@yandex-lego/serp-header/dist/metrika/notifier.desktop');
...
const scriptSrc = `${staticData.BASE_PATH}metrika/notifier/notifier-base.desktop.client.js`
```
...
`YENV=testing block=notifier platform=desktop npm run build:dev`
Они соберутся в корне `islands` в `desktop.examples/notifier`.
Можно увидеть их, запустив в корне `npx http-server` и открыв http://127.0.0.1:8080/desktop.examples/notifier.
