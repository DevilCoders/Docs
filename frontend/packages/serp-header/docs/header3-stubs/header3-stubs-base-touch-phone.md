## Блок `header3-stubs` на touch-phone
### Использование
```javascript
const dist = require('@yandex-lego/serp-header/dist/base/header3-stubs.touch-phone');
...
`YENV=testing block=header3-stubs platform=touch-phone npm run build:dev`
Они соберутся в корне `islands` в `touch-phone.examples/header3-stubs`.
Можно увидеть их, запустив в корне `npx http-server` и открыв http://127.0.0.1:8080/touch-phone.examples/header3-stubs.
