...
### Использование
```javascript
const dist = require('@yandex-lego/serp-header/dist/products/header3.touch-phone');
...
const content = dist.getContent({
    ...
    ctx: {
        /*  При сборке модификаторы заменяются на header3_scrolled_ + (...), а нам нужен модификатор true. */
         ...
         /*   Показываем Плюс, если передан в поле yaplusHTML */
         (ctx.yaplusHTML || ""),

         /*  Показываем Избранное, если передано в поле favoritesHTML */
         (ctx.favoritesHTML || ""),

         /*  Показываем Колокол, если передан в поле notifierHTML */
         (ctx.notifierHTML || ""),

         /*  Показываем Иконку пользователя, если передана в userHTML, иначе - кнопку Войти */
         (ctx.userHTML ? ctx.userHTML : (ctx.loginHTML || "")),
