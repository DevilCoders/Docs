## Блок `tools-cross-navigation` на desktop
### Использование
```javascript
const dist = require('@yandex-lego/serp-header/dist/base/tools-cross-navigation.desktop');
...
const scriptSrc = `${staticData.BASE_PATH}base/tools-cross-navigation/tools-cross-navigation-base.desktop.client.js`
```
...
`YENV=testing block=tools-cross-navigation platform=desktop npm run build:dev`
Они соберутся в корне `islands` в `desktop.examples/tools-cross-navigation`.
Можно увидеть их, запустив в корне `npx http-server` и открыв http://127.0.0.1:8080/desktop.examples/tools-cross-navigation.
