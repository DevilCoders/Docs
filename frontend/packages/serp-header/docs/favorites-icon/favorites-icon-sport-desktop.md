## Блок `favorites-icon` на desktop
### Использование
```javascript
const dist = require('@yandex-lego/serp-header/dist/sport/favorites-icon.desktop');
...
`YENV=testing block=favorites-icon platform=desktop npm run build-examples`
Они соберутся в корне `islands` в `desktop.examples/favorites-icon`.
Можно увидеть их, запустив в корне `npx http-server` и открыв http://127.0.0.1:8080/desktop.examples/favorites-icon.
