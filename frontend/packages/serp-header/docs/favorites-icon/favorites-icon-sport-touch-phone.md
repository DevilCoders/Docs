## Блок `favorites-icon` на touch-phone
### Использование
```javascript
const dist = require('@yandex-lego/serp-header/dist/sport/favorites-icon.touch-phone');
...
const content = dist.getContent({
     ...
     */
    ctx: {
        /*  Ссылка на Коллекции, по умолчанию //yandex.ru/collections */
         favoritesUrl,
...
`YENV=testing block=favorites-icon platform=touch-phone npm run build-examples`
Они соберутся в корне `islands` в `touch-phone.examples/favorites-icon`.
Можно увидеть их, запустив в корне `npx http-server` и открыв http://127.0.0.1:8080/touch-phone.examples/favorites-icon.
