...
### Использование
```javascript
const dist = require('@yandex-lego/serp-header/dist/base/favorites-icon.touch-phone');
...
const content = dist.getContent({
     ...
     */
    ctx: {
        /*  Ссылка на Коллекции, по умолчанию //yandex.<tld>/collections */
         favoritesUrl,
...
`YENV=testing block=favorites-icon platform=touch-phone npm run build:dev`
