## Блок `tools-settings` на desktop
### Использование
```javascript
const dist = require('@yandex-lego/serp-header/dist/base/tools-settings.desktop');
...
const scriptSrc = `${staticData.BASE_PATH}base/tools-settings/tools-settings-base.desktop.client.js`
```
...
`YENV=testing block=tools-settings platform=desktop npm run build:dev`
Они соберутся в корне `islands` в `desktop.examples/tools-settings`.
Можно увидеть их, запустив в корне `npx http-server` и открыв http://127.0.0.1:8080/desktop.examples/tools-settings.
