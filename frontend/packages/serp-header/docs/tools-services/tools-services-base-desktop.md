## Блок `tools-services` на desktop
### Использование
```javascript
const dist = require('@yandex-lego/serp-header/dist/base/tools-services.desktop');
...
const scriptSrc = `${staticData.BASE_PATH}base/tools-services/tools-services-base.desktop.client.js`
```
...
`YENV=testing block=tools-services platform=desktop npm run build:dev`
Они соберутся в корне `islands` в `desktop.examples/tools-services`.
Можно увидеть их, запустив в корне `npx http-server` и открыв http://127.0.0.1:8080/desktop.examples/tools-services.
