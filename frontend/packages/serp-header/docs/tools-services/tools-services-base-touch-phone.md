## Блок `tools-services` на touch-phone
### Использование
```javascript
const dist = require('@yandex-lego/serp-header/dist/base/tools-services.touch-phone');
...
const scriptSrc = `${staticData.BASE_PATH}base/tools-services/tools-services-base.touch-phone.client.js`
```
...
`YENV=testing block=tools-services platform=touch-phone npm run build:dev`
Они соберутся в корне `islands` в `touch-phone.examples/tools-services`.
Можно увидеть их, запустив в корне `npx http-server` и открыв http://127.0.0.1:8080/touch-phone.examples/tools-services.
