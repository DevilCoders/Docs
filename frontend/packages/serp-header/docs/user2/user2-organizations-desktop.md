## Блок `user2` на desktop
### Использование
```javascript
const dist = require('@yandex-lego/serp-header/dist/organizations/user2.desktop');
...
const content = dist.getContent({
    ...
    ctx: {
        /*  uid пользователя */
         ...
         /*  Секретный ключ (как при смене языка) */
         secretKey,

         /*  Ручка для получения списка организаций, по умолчанию https://api.directory.yandex.{tld}/v6/organizations-by-session */
         organizationsUrl,

         /*  Ручка для смены контекста организаций, по умолчанию https://connect.yandex.{tld}/portal/context?mode=portal */
         contextSwitchUrl,

         /*  Ручка для добавления организаций, по умолчанию https://connect.yandex.{tld}/portal/registration?action=add */
         addOrganizationUrl,
...
const scriptSrc = `${staticData.BASE_PATH}organizations/user2/user2-base.desktop.client.js`
```
