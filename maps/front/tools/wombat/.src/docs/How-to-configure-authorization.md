...
[resources tab in ABC](https://abc.yandex-team.ru/services/maps-front-{{ProjectName}}/resources/) and create
three new TVM resources named
- `maps-front-{{ProjectName}}-prod`
- `maps-front-{{ProjectName}}-test`
- `maps-front-{{ProjectName}}-dev`

The secrets content is located by [Vault](https://vault.yandex-team.ru/) links contained in
`maps-front-{{ProjectName}}-prod` and `maps-front-{{ProjectName}}-test` resources you have just created.
...
The last resource `maps-front-{{ProjectName}}-dev` is used for authorization in developing mode. The point is that
...
`maps-front-{{ProjectName}}-dev` resource and `self_tvm_id` is its id.
