...
Как было раньше можно прочитать в [README_OLD](README_OLD.md)



## Проекты

Каждый активный проект спецов живет как отдельный сервис в `maps/front/services`
...
4. Запуск проекта локально - `make dev`, приложение доступно по адресу https://localhost:8080/ (*Проверьте, что в CONTRIBUTING.md прописался DNS адрес, совпадающий с DNS адресом, указаным в сертификате при выполнении make local*)
...
Для загурзки картинок в аватарницу используем [upload-api](https://a.yandex-team.ru/arc_vcs/maps/front/services/upload-api) ([описание ручки](https://a.yandex-team.ru/arc_vcs/maps/front/services/upload-api/docs/v1/spec.yaml)). Пример отправки файла через upload-api в [спецпроекте Уралсиб](https://github.yandex-team.ru/maps/specprojects/blob/8a6c9ff29c1d369dd251cc6eca45d4ff31cd673e/projects/uralsib-admin/src/client/components/pages/moderation-page/moderation-page.tsx#L60).

### Мойра
[Мойра](https://a.yandex-team.ru/arc_vcs/maps/front/services/moira-intt) используется для хранения данных спецпроектов. Внутри доступ к проекту разграницен по `TVM id`. 

Для добавления нового проекта в Мойру нужно: 
- закоммитить в конфиг новый проект и его `tvm id`. Пример такого коммита [тут](https://a.yandex-team.ru/arc_vcs/commit/d36d5cbe251916030f1d2b46c3af25dbbc846127)
...
- при создании проекта с wombat в CONTRIBUTING лежит неверный локальный адрес, првоерьте, что он совпадает с DNS адресами в сертификате
...
1) Посмотрите в аркадии текущую версию приложения (версия в .qtools.json).
2) Решите, какое будет следующее обновление: `patch/minor/major`.
3) arc checkout -b your-branch
4) В ветке
   - `make patch/minor/major` (в зависимости от решения в п.2) - поднять версию
   - сделать деплой в тестинг и прод руками, или настроить ci
   - мерж ветки в транк
5) Смержите ветку в trunk
