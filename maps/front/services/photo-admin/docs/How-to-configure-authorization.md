...
[resources tab in ABC](https://abc.yandex-team.ru/services/maps-front-photo-admin/resources/) and create
three new TVM resources named
- `maps-front-photo-admin-prod`
- `maps-front-photo-admin-test`
- `maps-front-photo-admin-dev`

The secrets content is located by [Vault](https://vault.yandex-team.ru/) links contained in
`maps-front-photo-admin-prod` and `maps-front-photo-admin-test` resources you have just created.
...
The last resource `maps-front-photo-admin-dev` is used for authorization in developing mode. The point is that
...
`maps-front-photo-admin-dev` resource and `self_tvm_id` is its id.
