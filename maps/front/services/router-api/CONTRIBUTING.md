...
1. Go to the project root
   ...
   ```sh
   cd <arcadia-path>/maps/front/services/router-api
   ```
...
5. Build and run the service in development mode:
   ...
   make dev.setup
   make build-watch
   make dev # in a seperate console
   ```
...
If there are changes to the typings of the responses, one can easily update the specs in the API docs by running:
```
make generate-response-docs
```
