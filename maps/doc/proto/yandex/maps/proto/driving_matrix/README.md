...
| srcdir | dir1~dir2~...dirN | массив направлений привязки точек (угол на север, по часовой стрелке), пустое значение - любое направление |
| dstll | lon1,lat1~lon2,lat2~...lonN,latN | массив destination точек |
| dstdir | dir1~dir2~...dirN | массив направлений привязки точек (угол на север, по часовой стрелке), пустое значение - любое направление |
| projection_mode | smart, nearest, multi | режим привязки точек к ребрам дорожного графа, по умолчанию smart |
...
| disable_jams | true или false | отключает использование realtime jams диффа, по умолчанию false |
| disable_slices | true или false | отключает использование диффов по историческим данным, по умолчанию false |
| disable_scaling | true или false | отключает коррекцию времени проезда, по умолчанию false |

| заголовок запроса |
|---|
| **Accept**:</br>application/x-protobuf (по умолчанию)</br>text/x-protobuf</br>application/json |

| код ответа  | тело ответа | заголовок ответа |
|---|---|---|
| 200 | proto-объект [Matrix](https://a.yandex-team.ru/arc/trunk/arcadia/maps/doc/proto/yandex/maps/proto/driving_matrix/matrix.proto) | **Content-Type**:</br>application/x-protobuf</br>text/x-protobuf</br>application/json |
...
| dtm | uint32 | время отправления в unixtime, по умолчанию текущее время |
| projection_mode | smart, nearest, multi | режим привязки точек к ребрам дорожного графа, по умолчанию smart |
| config_id | uint64 | идентификатор конфигурации исторических дифов, по умолчанию не учитывается |
| avoid | tolls | для построения матриц по бесплатным маршрутам, по умолчанию маршруты могут быть платными |
| vehicle_type | car, taxi или truck | тип ТС, по умолчанию car |
| vehicle_weight | float | вес ТС, тонны, по умолчанию не учитывается |
| vehicle_max_weight | float | макс. вес ТС, тонны, по умолчанию не учитывается |
...
| параметр запроса | тип/формат/допустимые значения | описание |
|---|---|---|
| token | string | токен асинхронной задачи |
...
| параметр запроса | тип/формат/допустимые значения | описание |
|---|---|---|
| dtm_from | int64 | начало временного интервала в unixtime|
| dtm_to | int64 | конец временного интервала в unixtime, интервал - полузакрытый |
...
| код ответа  | тело ответа | заголовок ответа |
|---|---|---|
| 200 | json-объект cо списком адресов | **Content-Type**:</br>application/json |
| 400 | Описание проблемы с переданными параметрами | **Content-Type**:</br>text/plain |
| 500 | Ошибка сервера при обработке запроса, следует повторить запрос на данную ручку | **Content-Type**:</br>text/plain |
...
| код ответа  | тело ответа | заголовок ответа |
|---|---|---|
| 200 | proto-объект [ProjectionResult](https://a.yandex-team.ru/arc/trunk/arcadia/maps/doc/proto/yandex/maps/proto/driving_matrix/projection.proto) | **Content-Type**:</br>application/x-protobuf</br>text/x-protobuf</br>application/json |
| 400 | Описание проблемы с переданными параметрами | **Content-Type**:</br>text/plain |
| 500 | Ошибка сервера при обработке запроса, следует повторить запрос на данную ручку | **Content-Type**:</br>text/plain |
