...
The task checks input tables have been already created and output dataset has not been uploaded to ecstatic yet. If it so, task will execute build_edge_seq_stat.
...
For update build_edge_seq_stat:
- build for Ubuntu 18.04 LTS
    > cd build_edge_seq_stat
    > ya make -r
- upload to Sandbox:
    > ya upload --type MAPS_EDGE_SEQ_STAT_BUILDER --ttl=inf ./build_edge_seq_stat
