# Получение токена по UID пользователя
...
В исключительных случаях токены можно получать, указав только UID пользователя. Этот способ доступен только из сети Яндекса и защищен грантами, которые можно запросить в рассылке [oauth@](oauth@yandex-team.ru).
...
Чтобы получить токен по UID, следует указать значение <q>assertion</q>. ||
|| `assertion` {#assertion} | UID пользователя, для которого нужно выписать OAuth-токен. ||
...
Часто встречающиеся коды и описания ошибок приведены в таблице:

#|
|| **HTTP-код ответа** | **Код ошибки** | **Описание** ||
|| 400 | invalid_request | Неверный формат запроса (не указан обязательный параметр и т. п.). ||
|| 400 | invalid_grant | UID, указанный в значении параметра [assertion](#assertion), не найден или не является числом. ||
