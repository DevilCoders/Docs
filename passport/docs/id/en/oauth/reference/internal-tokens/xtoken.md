...
Using the OAuth token with the **X-Token issue** right (hereinafter “X-token”), you can get a new token for the same Yandex account. This method is only used for mobile apps and is implemented via [Account Manager](https://wiki.yandex-team.ru/yandexmobile/accountmanager).

By default, the **Issuing X-Token** right is hidden for all Yandex.OAuth users. You can request access to it in the [oauth@](mailto:oauth@yandex-team.ru) mailing list. After that, you can select this right on the app settings page.
...
Format for requesting a token in exchange for an X-token:
...
When requesting a token in exchange for an X-token, specify the “x-token” value. ||
|| `access-token` | OAuth token that can be used to get other tokens.

You can receive these tokens only if the **Issuing X-Token** right is selected in the app settings. By default, this right is hidden for all Yandex.OAuth users. You can request access to it in the [oauth@](mailto:oauth@yandex-team.ru) mailing list. ||
