...
The OAuth token issued for an account is inconvenient for some server interaction scenarios. For example, tokens may be needed to request data about blocked accounts, but you can't get a valid token for such accounts.

In this case, you can request a depersonalized token (see the [OAuth 2.0 specification](http://tools.ietf.org/html/rfc6749#section-4.4)). These tokens aren't related to any account and are only used for the mutual authorization of Yandex server apps.

{% note info "Restriction" %} 

We only recommend issuing short-lived tokens (with a lifetime of up to 60 seconds) to reduce the security risk for the service. To limit the token lifetime for new or existing rights, write to the [oauth@](mailto:oauth@yandex-team.ru) mailing list.
...
Request format for a depersonalized token:
...
----- | -----
`grant_type` | A method used to request a token. To get a token by UID, specify the “client_credentials” value.
...
`client_secret` | App password. Available in [app properties]{% if lang == "en" %}(https://oauth.yandex.com/){% endif %} (click the app name to open its properties). This parameter is required if it wasn't specified in the [authorization header](#d19e40) request.
