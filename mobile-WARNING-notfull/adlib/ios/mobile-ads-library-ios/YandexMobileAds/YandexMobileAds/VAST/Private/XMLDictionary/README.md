...
This property controls how XML attributes are handled. The default is `YMAXMLDictionaryAttributesModePrefixed` meaning that attributes will be included in the dictionary, with an _ (underscore) prefix to avoid namespace collisions. Alternative values are `YMAXMLDictionaryAttributesModeDictionary`, which will place all the attributes in a separate dictionary, `YMAXMLDictionaryAttributesModeUnprefixed`, which includes the attributes without prefix (which may cause collisions with nodes) and `YMAXMLDictionaryAttributesModeDiscard`, which will strip the attributes.

    @property (nonatomic, assign) XMLDictionaryNodeNameMode nodeNameMode;

This property controls how the node name is handled. The default value is `YMAXMLDictionaryNodeNameModeRootOnly`, meaning that the node name will only be included in the root dictionary (the names for the children can be inferred from the dictionary keys, but the `nodeName` method won't work for anything except the root node). Alternative values are `YMAXMLDictionaryNodeNameModeAlways`, meaning that the node name will be included in the dictionary with the key `__name` (and can be accessed using the `nodeName`) method, or `YMAXMLDictionaryNodeNameModeNever` which will never include the `__name' key.
...
The above examples assumes that you are using the default setting for `collapseTextNodes` and `alwaysUseArrays`. If `collapseTextNodes` is disabled then you would instead access `<foo>`'s value by writing:
	...
	NSString *foo = [[xmlDoc valueForKeyPath:@"bar.foo"] yma_innerText];
...
If the `alwaysUseArrays` option is enabled then would use one of the following, depending on the `collapseTextNodes` property:
    ...
    NSString *foo = [[[xmlDoc valueForKeyPath:@"bar.foo"] firstObject] yma_innerText];

To get the cliche attribute of `bar`, you could write:

    NSString *barCliche = [xmlDoc[@"bar] attributes][@"cliche"];

If the `attributesMode` is set to the default value of `YMAXMLDictionaryAttributesModePrefixed` then you can also do this:

	NSString *barCliche = [xmlDoc valueForKeyPath:@"bar._cliche"];

Or if it is set to `YMAXMLDictionaryAttributesModeUnprefixed` you would simply do this:
