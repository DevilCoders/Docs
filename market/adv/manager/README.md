...
1. `ya make -tt`
2. `madv-manager --help`
3. `madv-manager -c config.yaml runserver`
4. `madv-manager version`
...
ya ide vscode-go -P ~/manager-project -j8 market/adv/manager
...
3. `yarn build`
  ...
  # specFile: /app/conf/madv-manager/swagger.yaml