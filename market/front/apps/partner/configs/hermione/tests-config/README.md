# partner-hermione-tests-config

Содержит списки скипов hermione тестов для репозитория [market/partnernode](https://github.yandex-team.ru/market/partnernode).
...
Символом `=` раньше помечались тикеты в релизе (слитно с тикетом, `=MARKETPARTNER-xxxxx`).
Такие пропуски игнорируются в релизных прогонах.
Сейчас тикеты в релизе размечаются __автоматически__ внутри запуска АТ.
...
При клонировании этого репозитория отдельно от partnernode для запуска инструмента очистки
