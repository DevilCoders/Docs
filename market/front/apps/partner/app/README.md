...
- [Pages](#pages)
  - [BasePage](#basepage)
    ...
    - [HtmlPage](#htmlpage)
  - [BasePlatformPage](#baseplatformpage)
    - [HtmlPlatformPage](#htmlplatformpage)
...
* `debug` — инстанс класса `Debug`, проинициализированный для текущего запроса (также доступен через `this.debug`);
...
Предоставляет api:
* `<String> renderHtml(templatePath, data)` — возвращает результат шаблонизации, замеряет время.

Методы, доступные для переопределения / расширения:
* `<Object> getTemplateData()` — возвращает итоговые данные для передачи в шаблон (`this.registry` + `{tasks: this.getResults()}`).
...
Расширяет: [BasePlatformPage](#baseplatformpage)

Практически идентичен [HtmlPage](#htmlpage), но предназначен для страниц под коконом.
