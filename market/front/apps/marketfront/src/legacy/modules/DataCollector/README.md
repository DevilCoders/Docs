...
Какие функции в проекте выполняет:
- описание cms интерфеса к ручкам бекенда
- переиспользуемость (за счет одинакового формата ответа, разные виджеты могут использовать разные гарсоны)
- возможность замешивания выдач нескольких гарсонов, пейджинга

  // пейджинг сейчас почти везде со стороны бекенда выполняется
- подгрузка с клиента в виджетах
   - через механизм с memento (последнее состояние вызова гарсона сжатое в строку),
   - единым епиком (marketfront/src/epics/widgets/garson.js)
