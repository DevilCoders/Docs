# FilterValuesRadioList

Компонент для отображения списка значений `radio`-, `enum`-, `boolean`-фильтра в виде radio-группы значений. 
...
```jsx
const initialState = require('@self/platform/components/FilterValuesRadioList/__fixtures__/filter.json');
...
<div>
    Нужен корректный мок! Кажется компонент deprecated
    <FilterValuesRadioList 
        filterId={state.id}
        values={state.values}
