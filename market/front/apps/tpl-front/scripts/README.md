...
Build/Start и любой скрипт, выполнение которого приводит к выкачке зависимостей:
  ...
  - `SKUBI_REPO` URI репозитория десктопного Маркета
  - `SKUBI_REV` Ревизия репозитория [MarketNode][MarketNode]
  - `*_SCHEME` Схема выкачки зависимости, например `git+ssh` или `git+https` и .т.д.

Start:

  - `start_socket_addr` Сокет, который будет прослушивать приложение.
...
[MarketNode]: https://github.yandex-team.ru/market/MarketNode
