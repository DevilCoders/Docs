# Лендинг для партнёрской сети Яндекс.Маркета
...
`npm run dev` - Стартует сервер на порте 3000, запускает watch режим, отслеживает изменения исходных файлов

`npm run release` - Сборка релизной версии для выкладки

P.S.: Рекомендуемая для сборки версия Node.js 10.14.1, но все нормально собралось и для 8.9.0.


## Как сделать и выкатить изменения

1. Клонируем репозиторий https://github.yandex-team.ru/market/market-static-pages
2. Переходим в папку с исходниками нашего сайта - https://github.yandex-team.ru/market/market-static-pages/tree/master/src/affiliate-promo/welcome
...
8. Чтобы выложить release-версию в прод, клонируем sandbox таску BUILD_MARKET_FRONT_STATIC ([ссылка](https://sandbox.yandex-team.ru/task/868133396/view))
...
Адрес тестинга - https://marketaff.tst.vs.market.yandex.net/
...
https://a.yandex-team.ru/arc/trunk/arcadia/market/sre/nanny/front/affiliate/tmpl/conf/nginx/sites-enabled/103-marketaff.conf

## Про аутсорс

Автор - Astroshock

Оригинал исходников - https://st.yandex-team.ru/MARKETAFFILIATE-1872#5d77ea88122154001d7b0853

### Отличия от оригинала исходников

- Код проекта поместил в директорию src, поправил соответсвующие пути в gulpfile.js и nunjucks.config.js
- Почистил локальный .gitignore с учетом настроек глобального .gitignore на весь репозиторий
- Причесал package.json, добавил команды для сборки, согласно правилам оформления пакетов в репозитории [market-static-pages](https://github.yandex-team.ru/market/market-static-pages) 
