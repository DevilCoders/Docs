...
* `<Response> resource({String} id, {Object} [params], {Boolean} [isMandatory=true])` — возвращает результат вызова ресурса с переданными и контекстными (contextId);
...
* при включенном в конфиге режиме `debug` и наличия в запросе параметра `dd` добавить в шаблон `{errors: this.getTasksErrors()}` и флаг `dump`;
