...
Определяемся, какую группу доступа из [resolverGroups](https://github.yandex-team.ru/market/partnernode/blob/master/b2b-core/shared/constants/resolverGroups.ts) будет использовать резолвер
...
>
> Но если в [паттерне страницы, с которой вызывается резолвер](https://github.yandex-team.ru/market/partnernode/blob/master/shared/utils/createSafeResolver/README.md#как-понять-какой-кабинет-нам-нужен) нет параметра `<platformType>`,
...
>
> ```javascript
> cabinetTypeMapper: (ctx, params) => params.platformParam;
> ```
