...
Возможна кастомизация следующих переменных окружения(в скобках указано значение по умолчанию):
  ...
  - `_webapp_package_name`(yandex-market-x5-view) :: имя пакета, в который будет упаковано веб приложение
  - `_webapp_separate_slot`(false) :: если true, то запишет версию в имя пакета, заменив `.` на `-`. Требует указания `_webapp_destdir`
  - `_webapp_slot_name` :: если сборка происходит с `_webapp_separate_slot = true`, то значение переменной будет дописано в конец версии. Пример того что получится если эта переменная будет равна `o-lo-lo`: ```yandex-market-x5-view-17905-028284a1-o-lo-lo_17905.028284a1.o.lo.lo-1_all.deb```
  Обратите внимание на версию пакета, знаки `-` были заменены на `.`.
  - `_webapp_branch`(develop) :: ветка из которой будет собираться пакет
  - `_webapp_destdir`(/usr/local/www5/market для `_webapp_package_name=yandex-market-x5-view` и `/usr/local/www5/<_webapp_package_name_webapp_package_name>` в ином случае) :: целевая директория, в которую будут сложены файлы веб приложения
  - `_static_package_name`(в данный момент не используется, см. [статический пакет](README.md#%D0%A1%D1%82%D0%B0%D1%82%D0%B8%D1%87%D0%B5%D1%81%D0%BA%D0%B8%D0%B9-%D0%BF%D0%B0%D0%BA%D0%B5%D1%82)) :: имя пакета, в который будет упакована статика веб приложения
...
Дополнительные переменные окружения, которые влияют на процесс сборки:
  - `COMMON_REV` ветка [common][common-repo]
  - `VENDOR_REV` ветка [market-vendor-libs][market-vendor-libs]
  - `YENV`(production) :: Целевое окружение для которого выполняется сборка
  - `PACKAGING`(yes) :: Если мы собираем пакет, то она должна иметь значение `yes`, в противном случае будут выкачиваться export, export-static репозитории, которых не должно быть в пакете, выкладываемом куда-либо(эти штуки выкладываются отдельными пакетами)
...
Под классическим релизным пакетом я понимаю пакет, имя которого `yandex-market-x5-view`.
...
`yandex-market-x5-view_17805.01484f1-1_all.deb`

Он будет ставиться в `/usr/local/www5/market`.
...
Допустим мы хотим собрать эксперимент с именем `experiment-name`. Нам потребуются:
...
```bash
COMMON_REV=develop \
    _webapp_package_name=yandex-market-x5-view-exp \
    _webapp_branch=exp/experiment-name \
    _webapp_separate_slot=true \
    _webapp_slot_name=experiment-name \
    _webapp_destdir=/usr/local/www5/market-exp/experiment-name \
    ../runner -g <id вашего gpg ключа> -c
```

На выходе получится пакет(версии в вашем случае будут отличаться)
`yandex-market-x5-view-exp-17905-028284a1-experiment-name-1_17905.028284a1.experiment.name-1_all.deb`

Он будет ставиться в `/usr/local/www5/market-exp/experiment-name`, как мы и указали при запуске сборки.
Удостовериться в этом можно с помощью команды `dpkg -c package.deb | less`(советую использовать less, т.к. файлов в пакете много).
