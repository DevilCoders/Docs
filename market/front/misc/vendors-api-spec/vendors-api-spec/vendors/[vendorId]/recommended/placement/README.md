## PUT /vendors/[vendorID]/recommended/placement

~~Изменение размещения (включение/выключение) услуги "Рекомендованные магазины" для указанного вендора~~
...
### Ответ:
  ...
  - result `<Object>`
    - **item** `<Object>`
      - **placement** `ACTIVE|SUSPENDED|REJECTED` - текущий статус размещения (ACTIVE - размещается, SUSPENDED - не размещается, REJECTED - размещение невозможно)
      - **activeCutoffTypes** [`Array[CutoffType]`](/_entities/CutoffType.md) - список активных типов отключений, висящих на кампании **после** попытки смены типа размещения.
  - errors `<Error[]>`

### Примечания
  - Если был послан запрос на отключение (`SUSPENDED`) кампании, то ответ будет 
