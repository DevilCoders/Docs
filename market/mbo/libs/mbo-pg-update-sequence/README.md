Последовательность изменений через триггеры в БД
================================================

Во многом это продолжение идей pg-log-id.
...
При этом пробуем учесть работу PG. Простой подход, где есть таблица (ключ => последовательность) приводит к рандомным обновлениям страниц данных. Чтобы этого избежать тут используется +/- write-only подход.

Таблицы:
- _update_staging(update_id, keys, update_ts) - накапливает изменения и из неё происходят insert/delete-ы в большом количестве, в пределе, если изменений нет, она становится пустой. Тут основные риски по autovacuum-у.
- _update_seq(update_seq, keys, update_ts) - в неё перекладываются данные из _update_staging в один поток и тут уже гарантируем последовательность update_seq.
- для эффективной очистки _update_seq - партиционируем её на диапазоны update_seq, старые партиции удаляются примерно по таймстемпу обновления первой записи.
