...
- Янг/Толока - имеют хорошую поддержку в приложении, но не являются ядром, вокруг которого всё работает, следствия:
    ...
    - но, скорее всего, будет несколько сложнее кроссприоритезировать группы или делать что-то сложное со связью
      янг+задание

Локальный запуск из IDEA:
=========================

- Создать файл `${ARCADIA_ROOT}/market/mbo/markup3/src/main/properties.d/local/{50..99}_local.properties` с содержимым:
    ``` properties
    markup.yang.api.token=TOKEN
    markup.toloka.api.token=TOKEN
    markup.yt.token=TOKEN
    markup.solomon.oauth.token=TOKEN
    ```

Токены можно посмотреть [тут](https://yav.yandex-team.ru/secret/sec-01f98xrwbqpp4zkq9rfmw3nqmv/explore/versions)

- Создать конфигурацию запуска в IDEA:
    + Main class: [Markup3](./src/main/java/ru/yandex/market/markup3/Markup3.kt)
    + VM options:
      ``` properties
      -Dconfigs.path=${ARCADIA_ROOT}/market/mbo/markup3/src/main/properties.d 
      -Dlocal-pg=true
      ```
...
Обновление публичного proto
---------------------------
После того, как публичное proto-api маркапа обновится, нужно доставить потребителям новую версию библиотек
для подключения api к себе в проекты. Это можно сделать следующим образом:
1. Дождаться мержа в транк PR с правками api.
2. Получить номер ревизии мержа (появляется там же в PR вместо кнопки merge).
3. Сходить в пайплайн: https://tsum.yandex-team.ru/pipe/projects/mbo/delivery-dashboard/mbo-libs-arc
4. Добавить в пайплайне ревизию из пункта 2 руками через кнопку "добавить ревизию", если она еще не добавлена
5. Создать релиз из ревизии.

Этот пайплайн периодически может неожиданно падать, в таких случаях стоит сначала попробовать перезапустить упавший кубик.
