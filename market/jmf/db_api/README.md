# Базовые абстракции для работы с базой данных

**Важно:** дальше модифицировать market/lilucrm/docs/groovy/query.md

Модуль декларирует:
 
1. **[DbService](src/main/java/ru/yandex/market/jmf/db/api/test/DbService.java)** - сервис для взаимодействия с базой
   данных, для получения и сохранения бизнес-объектов в базу данных, Стоит обратить внимание, что изменение
   бизнес-объектов должно осуществляться только через *BcpService*, описанный в модуле *[bcp](../README.md)*.
1. **[DbScriptServiceApi](src/main/java/ru/yandex/market/jmf/db/api/test/impl/DbScriptServiceApi.java)** - скриптовое
   API для работы с БД (в частности, для выборки объектов из БД)
