...
`src/entries` - классы, которые используются "снаружи" из `bem`-мира;
...
`index.tsx` - главный файл, `entrypoint`, с него начинается сборка. В него нужно добавить компонент из `src/entries` и записать его в `window.metrika`, после чего компонент появится в `bem`-мире.
...
`make react-build` - соберет бандл в `metrika_3.0/build/js/_main_{lang}.js`. В `production` сборке так же будет собран `css` файл - `metrika_3.0/build/css/_main.css`;
...
### !!!Важно!!!
В перечисленных выше командах можно и нужно задать переменную окружения `LANGS`, передав ей список языков через запятую, например

    LANGS=ru make react-watch-dev

В ином случае будут собраны все 4 бандла (под каждый язык)
...
`npm run styleguide` - запустить [styleguidist](https://h.yandex-team.ru/?https%3A%2F%2Fgithub.com%2Fstyleguidist%2Freact-styleguidist) в режиме слежения за файлами и возможностью редактировать код примеров прямо в браузере;

`npm run test:watch` - запустить тесты [jest](https://h.yandex-team.ru/?https%3A%2F%2Ffacebook.github.io%2Fjest%2F) в режиме слежения за изменениями файлов.
