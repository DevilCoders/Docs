...
[registry](https://registry.yandex.net/maps_adv/export). 
Дальше необходимо обновить кофигурацию в qloud.

## Docker
Для сборки образа необходимо из корня аркадии выполнить следующую 
команду:
`ya package --docker --docker-repository maps_adv maps_adv/export/app/package.json`
...
## Qloud
Для этого процесса используется [qtools](https://github.yandex-team.ru/maps/infra/tree/master/packages/qtools). 
