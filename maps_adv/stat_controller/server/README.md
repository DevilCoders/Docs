...
[registry](https://registry.yandex.net/maps_adv/stat_controller). 
Дальше необходимо обновить кофигурацию в qloud.

## Docker
Для сборки образа необходимо из корня аркадии выполнить следующую 
команду:
`ya package --docker --docker-repository maps_adv maps_adv/stat_controller/server/app/package.json`
